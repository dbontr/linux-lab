...
test('QEMU browser pause blocks hot chains with offset-preserved guest clocks', () => {
  ...
  assert.doesNotMatch(controlSource, /linuxlab_pause_bh|linuxlab_resume_bh/)
  ...
  assert.match(controlPatchSource, /linuxlab_adjust_virtual_clock\(cpu_get_clock\(\)\)/)
  assert.match(controlPatchSource, /linuxlab_adjust_elapsed_ticks\(cpus_accel->get_elapsed_ticks\(\)\)/)
  assert.match(controlPatchSource, /linuxlab_adjust_elapsed_ticks\(cpu_get_ticks\(\)\)/)
