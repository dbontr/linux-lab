...
test('QEMU browser pause blocks hot chains in place and wakes directly on resume', () => {
  ...
  assert.match(controlSource, /linuxlab_clock_frozen/)
  assert.match(controlSource, /emscripten_atomic_wait_u32\(/)
  assert.match(controlSource, /ATOMICS_WAIT_DURATION_INFINITE/)
  assert.match(controlSource, /qatomic_store_release\(&linuxlab_paused, 0\);[\s\S]*emscripten_atomic_notify\(&linuxlab_paused, EMSCRIPTEN_NOTIFY_ALL_WAITERS\);[\s\S]*qemu_bh_schedule\(linuxlab_resume_bh_handle\)/)
  ...
  assert.doesNotMatch(controlPatchSource, /trysleepAnchor|cpuExecPath|tcg_tb_lookup/)
