...
test('QEMU browser pause blocks in-place at TCI/Wasm chain edges', () => {
  ...
  assert.match(controlSource, /uintptr_t linuxlab_pause_waiting_word_address\(void\)/)
  assert.match(controlSource, /void linuxlab_vcpu_pause_wait\(void\)/)
  ...
  assert.match(controlPatchSource, /const wasm32Path = process\.argv\[4\]/)
  assert.match(controlPatchSource, /const wasmTargetPath = process\.argv\[5\]/)
  assert.match(controlPatchSource, /void linuxlab_vcpu_pause_wait\(void\);/)
  ...
  assert.match(controlPatchSource, /linuxlab_pause_waiting_word_address/)
  assert.match(controlPatchSource, /i32\.atomic\.load/)
  assert.match(controlPatchSource, /i32\.atomic\.store/)
  assert.match(controlPatchSource, /memory\.atomic\.wait32/)
  assert.match(controlPatchSource, /drop wait result/)
  assert.match(controlPatchSource, /retry after a spurious wake/)
  assert.match(controlPatchSource, /tcg_wasm_out_linuxlab_pause_wait\(s\)/)
  assert.match(controlPatchSource, /tcg_wasm_out_op_br\(s, 2\); \/\/ br to the top of loop/)
  assert.match(controlPatchSource, /tcg_wasm_out_op_br\(s, 3\); \/\/ br to the top of loop/)
  assert.doesNotMatch(controlPatchSource, /trysleepAnchor|emscripten_sleep\(1\)|cpuExecPath|return EXCP_INTERRUPT|tcg_tb_lookup/)
  assert.match(buildSource, /tcg\/wasm32\.c/)
  assert.match(buildSource, /tcg\/wasm32\/tcg-target\.c\.inc/)
  assert.doesNotMatch(buildSource, /accel\/tcg\/cpu-exec\.c/)
  assert.doesNotMatch(source, /'-d', 'nochain'/)
  assert.doesNotMatch(controlSource, /emscripten_sleep\(|emscripten_thread_sleep|cpu->stop|cpu->stopped|cpu_resume\(cpu\)|cpu_exit\(cpu\)|qemu_cpu_kick\(cpu\)|CPU_FOREACH/)
