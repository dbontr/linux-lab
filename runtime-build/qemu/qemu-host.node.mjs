...
test('QEMU pause uses shared atomic state and keeps keyboard input on the owning thread', () => {
  ...
  assert.doesNotMatch(controlSource, /qemu_bh_schedule\(linuxlab_pause_bh_handle\)/)
  assert.doesNotMatch(controlSource, /qemu_bh_schedule\(linuxlab_resume_bh_handle\)/)
  assert.match(controlSource, /qemu_bh_schedule\(linuxlab_text_bh_handle\)/)
  assert.match(controlSource, /qatomic_store_release\(&linuxlab_paused, 1\)/)
  assert.match(controlSource, /qatomic_store_release\(&linuxlab_paused, 0\)/)
  assert.match(controlSource, /qatomic_load_acquire\(&linuxlab_paused\)/)
