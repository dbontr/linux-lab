...
test('QEMU browser pause uses the TCI dispatcher boundary and direct resume wake', () => {
  ...
  assert.match(controlSource, /emscripten_futex_wait\(&linuxlab_paused, 1, 1000\.0\)/)
  assert.match(controlSource, /qatomic_store_release\(&linuxlab_paused, 0\);[\s\S]*emscripten_futex_wake\(&linuxlab_paused, INT_MAX\);[\s\S]*qemu_bh_schedule\(linuxlab_resume_bh_handle\)/)
  assert.match(controlSource, /linuxlab_virtual_clock_ns/)
  assert.match(controlSource, /linuxlab_elapsed_ticks/)
  assert.doesNotMatch(controlSource, /cpu->stop|qemu_cpu_kick|cpu_exit\(cpu\)|cpu_resume\(cpu\)|vm_stop|vm_start|pause_all_vcpus|resume_all_vcpus/)
  assert.match(controlPatchSource, /linuxlab_vcpu_pause_wait\(\);/)
  assert.match(controlPatchSource, /linuxlab_pause_requested\(\)/)
  assert.match(controlPatchSource, /#define INSTANTIATE_NUM 2147483647/)
  assert.doesNotMatch(controlPatchSource, /wasmTargetPath|tcg_wasm_out_pause_requested|guardFastChain/)
  assert.match(buildSource, /"\$SOURCE_DIR\/tcg\/wasm32\.c"/)
  assert.match(buildSource, /"\$SOURCE_DIR\/tcg\/wasm32\.h"/)
