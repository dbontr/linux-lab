...
test('QEMU browser pause exits TCG before parking the vCPU and preserves guest clocks', () => {
  ...
  assert.match(controlPatchSource, /tcg_tb_lookup\(\(uintptr_t\)ctx\.tb_ptr\)/)
  assert.match(controlPatchSource, /return \(uintptr_t\)pause_tb \| TB_EXIT_REQUESTED/)
  assert.match(controlPatchSource, /cpu->exception_index = EXCP_INTERRUPT/)
  assert.match(controlPatchSource, /qemu_mutex_unlock_iothread\(\);\$\{mttcgEol\}            linuxlab_vcpu_pause_wait\(\);/)
  assert.doesNotMatch(controlSource, /emscripten_sleep\(/)
  assert.doesNotMatch(controlSource, /cpu_exit\(cpu\)|CPU_FOREACH|cpu->running/)
  assert.doesNotMatch(controlSource, /emscripten_atomic_wait|emscripten_futex/)
