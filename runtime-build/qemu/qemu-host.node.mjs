...
test('QEMU browser pause stays inside the qemu-wasm dispatcher yield boundary', () => {
  ...
  assert.match(controlSource, /void linuxlab_pause_waiting_set\(bool waiting\)/)
  assert.match(controlSource, /qatomic_load_acquire\(&linuxlab_pause_waiting\) == 0/)
  ...
  assert.doesNotMatch(controlPatchSource, /cpuExecPath|return EXCP_INTERRUPT|linuxlab_vcpu_pause_wait/)
  assert.match(controlPatchSource, /trysleepAnchor/)
  assert.match(controlPatchSource, /linuxlab_pause_waiting_set\(true\)/)
  assert.match(controlPatchSource, /while \(linuxlab_pause_requested\(\)\)/)
  assert.match(controlPatchSource, /emscripten_sleep\(1\)/)
  assert.match(controlPatchSource, /linuxlab_pause_waiting_set\(false\)/)
  assert.match(controlPatchSource, /tciGotoTbAnchor/)
  assert.match(controlPatchSource, /tciGotoPtrAnchor/)
  assert.match(controlPatchSource, /ctx\.tb_ptr = tb_ptr/)
  assert.match(controlPatchSource, /return 0/)
  assert.match(controlPatchSource, /guardFastChain/)
  ...
  assert.doesNotMatch(controlPatchSource, /tcg_wasm_out_op_br\(s, 4\)|pause_tb|tcg_tb_lookup/)
