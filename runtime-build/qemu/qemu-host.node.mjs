...
test('QEMU browser pause uses native vCPU stop state after cooperative TB-chain exit', () => {
  ...
  assert.match(controlPatchSource, /return EXCP_INTERRUPT/)
  assert.doesNotMatch(controlPatchSource, /linuxlab_vcpu_pause_wait/)
  ...
  assert.match(controlPatchSource, /uintptr_t pause_tb = \(uintptr_t\)ctx\.tb_ptr/)
  assert.match(controlPatchSource, /return pause_tb/)
  assert.match(controlPatchSource, /tcg_wasm_out_ctx_i32_load\(s, TB_PTR_OFF\)/)
  assert.match(controlPatchSource, /tcg_wasm_out_op_local_get\(s, TMP32_LOCAL_0_IDX\)/)
