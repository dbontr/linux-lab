...
test('QEMU boot arguments expose browser networking and OneDrive 9P', () => {
  ...
  assert.deepEqual(Array.from(args.slice(args.indexOf('-d'), args.indexOf('-d') + 2)), ['-d', 'nochain'])
...
test('QEMU browser pause uses cooperative no-chain boundaries and preserves guest clocks', () => {
  ...
  assert.doesNotMatch(controlPatchSource, /mttcgPath|wasm32Path|TB_EXIT_REQUESTED|cpu->exception_index/)
