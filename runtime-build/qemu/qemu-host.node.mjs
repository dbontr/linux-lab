...
test('QEMU boot arguments expose browser networking and OneDrive 9P', () => {
  ...
  const debug = args.indexOf('-d')
  assert.notEqual(debug, -1)
  assert.deepEqual(Array.from(args.slice(debug, debug + 2)), ['-d', 'nochain'])
...
test('QEMU browser pause parks the single TCG thread between unchained blocks', () => {
  ...
  assert.match(controlSource, /linuxlab_pause_waiters/)
  ...
  assert.doesNotMatch(controlSource, /cpu->stop|qemu_cpu_kick|cpu_resume|vm_stop|vm_start|pause_all_vcpus|resume_all_vcpus/)
  assert.match(controlPatchSource, /CF_NO_GOTO_TB \| CF_NO_GOTO_PTR/)
  assert.match(controlPatchSource, /linuxlab_vcpu_pause_wait\(\);/)
  assert.doesNotMatch(controlPatchSource, /wasm32|mttcg|trysleepAnchor|guardFastChain/)
  assert.match(controlPatchSource, /<accel\/tcg\/cpu-exec\.c>/)
  assert.match(buildSource, /"\$SOURCE_DIR\/accel\/tcg\/cpu-exec\.c"/)
