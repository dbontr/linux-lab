...
test('QEMU browser pause uses nonblocking native vCPU stop state and preserves guest clocks', () => {
  ...
  assert.match(controlPatchSource, /const mttcgPath = process\.argv\[4\]/)
  assert.match(controlPatchSource, /qemu_mutex_unlock_iothread\(\);\$\{mttcgEol\}            linuxlab_vcpu_pause_wait\(\);/)
  assert.doesNotMatch(controlPatchSource, /cpuExecPath|wasm32Path|TB_EXIT_REQUESTED|cpu->exception_index/)
