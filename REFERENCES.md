...
- [qemu-wasm generated-TB backend](https://github.com/ktock/qemu-wasm/blob/0ef7b4e2814b231705d8371dd7997f5b72e70baf/tcg/wasm32/tcg-target.c.inc) — compiled direct and indirect fast-chain backedges read the shared pause word and use WebAssembly atomic wait in place while Pause is active, preserving the generated TB call boundary and normal chaining while running.
...
- The QEMU browser profile is explicitly single-vCPU. Pause snapshots the adjusted virtual clock and elapsed ticks, sets a shared pause word, and parks TCI or compiled-Wasm chaining on that word with WebAssembly atomics. QEMU virtual-clock reads return the snapshot while paused; Resume advances per-clock offsets by the host pause duration, clears the pause word, and wakes the vCPU. The pause path does not call `cpu_disable_ticks()`, `cpu_exit()`, `qemu_cpu_kick()`, or QEMU's stopped-vCPU wait, and normal TB chaining remains enabled while running.
