...
- The QEMU browser profile is explicitly single-vCPU. Pause snapshots the adjusted virtual clock and elapsed ticks, sets a shared pause word, and parks the vCPU at qemu-wasm's C dispatcher with Emscripten's worker-safe atomic wait. Generated Wasm self-loops return to that dispatcher between iterations, while ordinary cross-TB chaining remains unchanged. Resume advances per-clock offsets by the host pause duration, clears the pause word, and notifies the vCPU. The pause path does not call `cpu_disable_ticks()`, `cpu_exit()`, `qemu_cpu_kick()`, or QEMU's stopped-vCPU wait.
