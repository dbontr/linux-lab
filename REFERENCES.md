...
- [qemu-wasm Wasm/TCI dispatcher](https://github.com/ktock/qemu-wasm/blob/0ef7b4e2814b231705d8371dd7997f5b72e70baf/tcg/wasm32.c) — qemu-wasm already yields its TCG dispatcher with `emscripten_sleep(0)`. Linux Lab reuses that Asyncify-safe dispatcher boundary: pause-aware TCI chains return to the dispatcher, which yields with `emscripten_sleep(1)` until Resume without returning through QEMU's CPU `sigsetjmp` boundary.
- [qemu-wasm generated-TB backend](https://github.com/ktock/qemu-wasm/blob/0ef7b4e2814b231705d8371dd7997f5b72e70baf/tcg/wasm32/tcg-target.c.inc) — compiled direct and indirect fast-chain backedges read the shared pause word and return to qemu-wasm's dispatcher with the current TB context intact when Pause is pending; normal chain branch depths remain unchanged.
...
- The QEMU browser profile is explicitly single-vCPU. Pause freezes QEMU's guest clocks and sets a shared pause word. TCI and compiled Wasm fast-chain backedges return to qemu-wasm's own dispatcher when that word is set; the dispatcher marks the vCPU paused and waits by reusing its existing Asyncify `emscripten_sleep()` mechanism. Resume restores the guest clocks and clears the pause word. The pause path never forces `cpu_exit()`, enters QEMU's stopped-vCPU condition wait, or crosses the CPU `sigsetjmp` boundary while suspended, and normal TB chaining remains enabled while running.
