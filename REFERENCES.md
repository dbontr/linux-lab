...
- [qemu-wasm CPU timers](https://github.com/ktock/qemu-wasm/blob/0ef7b4e2814b231705d8371dd7997f5b72e70baf/system/cpu-timers.c) — `cpu_disable_ticks()` and `cpu_enable_ticks()` preserve virtual clock and elapsed tick continuity; both require QEMU's Big Lock, so Pause disables ticks in a QEMU bottom half and Resume re-enables them only after the RR thread reacquires that lock.
- [QEMU TCG CPU execution loop](https://github.com/ktock/qemu-wasm/blob/0ef7b4e2814b231705d8371dd7997f5b72e70baf/accel/tcg/cpu-exec.c) — Linux Lab parks at the translation-block selection boundary and exits `cpu_exec()` with `EXCP_INTERRUPT` after the shared pause word is cleared.
- [QEMU single-thread TCG RR loop](https://github.com/ktock/qemu-wasm/blob/0ef7b4e2814b231705d8371dd7997f5b72e70baf/accel/tcg/tcg-accel-ops-rr.c) — the RR vCPU thread releases the Big Lock around `tcg_cpus_exec()` and reacquires it immediately afterward; Linux Lab finalizes Resume at that lock-safe boundary.
- [qemu-wasm Wasm/TCI dispatcher](https://github.com/ktock/qemu-wasm/blob/0ef7b4e2814b231705d8371dd7997f5b72e70baf/tcg/wasm32.c) — TCI direct and indirect chain edges consult the shared pause word so a pending Pause can leave a hot chain and reach the CPU execution boundary.
- [qemu-wasm generated-TB backend](https://github.com/ktock/qemu-wasm/blob/0ef7b4e2814b231705d8371dd7997f5b72e70baf/tcg/wasm32/tcg-target.c.inc) — compiled direct and indirect chain emitters read the same shared pause word and return from the generated TB when Pause is pending; normal chaining remains unchanged while running.
- [QEMU AIO bottom-half API](https://github.com/ktock/qemu-wasm/blob/0ef7b4e2814b231705d8371dd7997f5b72e70baf/include/block/aio.h) — the persistent Pause and text-input bottom halves move browser requests onto QEMU-owned execution without per-request QEMU allocation.
...
- The QEMU browser profile is explicitly single-vCPU. Pause is scheduled through a persistent QEMU bottom half that disables guest ticks under the Big Lock and sets a shared pause word. TCI and compiled-Wasm chain exits observe that word and reach the CPU execution loop, where the vCPU publishes its waiter count and parks. Resume clears and notifies the shared word from JavaScript; the awakened vCPU exits `cpu_exec()` with `EXCP_INTERRUPT`, the RR thread reacquires the Big Lock, re-enables guest ticks, and only then clears the waiter count reported to the UI. Normal TB chaining remains enabled while running.
- Browser-thread control entrypoints do not allocate QEMU objects. Pause and text input use persistent QEMU bottom halves; Resume mutates only pre-bound shared control words and never calls a QEMU Resume export while the vCPU is parked.
