...
- [qemu-wasm Wasm/TCI dispatcher](https://github.com/ktock/qemu-wasm/blob/0ef7b4e2814b231705d8371dd7997f5b72e70baf/tcg/wasm32.c) — cold TCI execution follows direct and indirect TB chains in C. Linux Lab inserts a shared-word wait at those chain edges so Pause blocks the vCPU without returning from TCG or invoking Asyncify.
- [qemu-wasm generated-TB backend](https://github.com/ktock/qemu-wasm/blob/0ef7b4e2814b231705d8371dd7997f5b72e70baf/tcg/wasm32/tcg-target.c.inc) — compiled direct and indirect fast-chain backedges use the same shared pause word and emit `memory.atomic.wait32` in-place before the existing branch, preserving the original TB return and structured-branch contracts.
...
- [Emscripten 3.1.50 atomic API](https://github.com/emscripten-core/emscripten/blob/3.1.50/system/include/emscripten/atomic.h) — `emscripten_atomic_wait_u32()` and `emscripten_atomic_notify()` map directly to WebAssembly `memory.atomic.wait32`/notify for worker-thread parking without Asyncify.
...
- The QEMU browser profile is explicitly single-vCPU. Pause freezes QEMU's guest clocks and sets a shared pause word. Cold TCI and compiled Wasm chain edges detect that word, mark the vCPU parked, and block directly with WebAssembly atomic wait while keeping the active TCG call stack intact. Resume restores the clocks, clears the pause word, and notifies the waiter so execution continues through the original chain. No `cpu_exit()`, QEMU stop-condition wait, Asyncify suspension, altered TB return, or branch-depth rewrite is required.
