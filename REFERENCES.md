...
- [qemu-wasm TCG region lookup](https://github.com/ktock/qemu-wasm/blob/0ef7b4e2814b231705d8371dd7997f5b72e70baf/tcg/region.c) — `tcg_tb_lookup()` maps qemu-wasm code pointers back to their owning `TranslationBlock` so Pause can use QEMU's native TB-exit encoding without losing guest-PC state.
- [qemu-wasm TCG CPU execution loop](https://github.com/ktock/qemu-wasm/blob/0ef7b4e2814b231705d8371dd7997f5b72e70baf/accel/tcg/cpu-exec.c) — `TB_EXIT_REQUESTED` handling and guest-PC synchronization contract used to leave TCG cleanly before the browser pause wait.
...
- [Emscripten 3.1.50 atomic API](https://github.com/emscripten-core/emscripten/blob/3.1.50/system/include/emscripten/atomic.h) — aligned WebAssembly atomic wait/notify operations used only after guest execution has returned to the MTTCG thread loop.
...
- QEMU browser Pause exits qemu-wasm at a translation-block boundary using the next TB pointer plus QEMU's requested-exit encoding, so QEMU synchronizes the exact guest PC and returns normally from `cpu_exec()`. The MTTCG thread then parks on an aligned WebAssembly atomic wait outside the TCG/setjmp stack; Resume clears and notifies the shared pause flag.
