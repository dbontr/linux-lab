...
- [qemu-wasm CPU timers](https://github.com/ktock/qemu-wasm/blob/0ef7b4e2814b231705d8371dd7997f5b72e70baf/system/cpu-timers.c) — `cpu_disable_ticks()` and `cpu_enable_ticks()` preserve virtual clock and elapsed tick continuity while the browser vCPU is paused.
- [QEMU TCG CPU execution loop](https://github.com/ktock/qemu-wasm/blob/0ef7b4e2814b231705d8371dd7997f5b72e70baf/accel/tcg/cpu-exec.c) — the translation-block loop provides a cooperative return point after a backend chain observes the pause word and before another guest block executes.
- [QEMU vCPU lifecycle](https://github.com/ktock/qemu-wasm/blob/0ef7b4e2814b231705d8371dd7997f5b72e70baf/system/cpus.c) — native `cpu->stop`, `cpu->stopped`, `qemu_wait_io_event()`, and `cpu_resume()` semantics used to park and wake the single browser vCPU without interrupting an active translation block.
- [QEMU TCG exit generation](https://github.com/ktock/qemu-wasm/blob/0ef7b4e2814b231705d8371dd7997f5b72e70baf/tcg/tcg-op.c) — direct `goto_tb` emits a numbered `exit_tb` fallback, while disabled indirect `goto_ptr` chaining uses `tcg_gen_exit_tb(NULL, 0)`; Linux Lab preserves those native exit contracts when Pause suppresses chaining.
- [qemu-wasm Wasm/TCI dispatcher](https://github.com/ktock/qemu-wasm/blob/0ef7b4e2814b231705d8371dd7997f5b72e70baf/tcg/wasm32.c) — cold-TB TCI direct chaining falls through to its existing `exit_tb` when Pause is set; indirect chaining returns the native null-TB exit.
- [qemu-wasm generated-TB backend](https://github.com/ktock/qemu-wasm/blob/0ef7b4e2814b231705d8371dd7997f5b72e70baf/tcg/wasm32/tcg-target.c.inc) — compiled direct chaining is skipped while Pause is set so the generated block reaches its normal `exit_tb`; indirect chaining uses the native null-TB return.
...
- The QEMU browser profile is explicitly single-vCPU. Both qemu-wasm TCI and generated direct/indirect TB chaining paths read Linux Lab's shared pause word before chaining. A Pause request sets QEMU's native vCPU stop request without kicking the active worker. Direct `goto_tb` chaining falls through to its existing `exit_tb`; indirect `goto_ptr` uses the native null-TB exit. QEMU then records the vCPU as stopped and waits on its normal halt condition. Resume restores the guest clocks, clears the pause word, and uses `cpu_resume()` to release the vCPU. Normal TB chaining stays enabled while running, and no custom sleep loop, Asyncify suspension, asynchronous active-vCPU exit, or blocking all-vCPU stop is required.
