...
- [QEMU TCG CPU execution loop](https://github.com/ktock/qemu-wasm/blob/0ef7b4e2814b231705d8371dd7997f5b72e70baf/accel/tcg/cpu-exec.c) — the normal translation-block loop provides the blocking pause point after a backend chain has returned without requiring an asynchronous CPU exit.
...
- [Emscripten 3.1.50 atomic API](https://github.com/emscripten-core/emscripten/blob/3.1.50/system/include/emscripten/atomic.h) — `emscripten_atomic_wait_u32()` and `emscripten_atomic_notify()` map directly to WebAssembly memory wait/notify instructions for a blocking worker pause without Asyncify or proxy-call processing.
...
- The QEMU browser profile is explicitly single-vCPU. Both qemu-wasm TCI and generated direct/indirect TB chaining paths read Linux Lab's shared pause word before chaining. A Pause request therefore lets the active TB finish normally, breaks the chain at its boundary, and then parks the vCPU in QEMU's ordinary execution loop with WebAssembly atomic wait. Resume restores the guest clocks, clears the pause word, and notifies the worker. Normal TB chaining stays enabled while running, and no Asyncify suspension, asynchronous `cpu_exit()`, signal, or blocking all-vCPU stop is required.
