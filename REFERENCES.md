...
- [qemu-wasm WebAssembly TCG dispatcher](https://github.com/ktock/qemu-wasm/blob/0ef7b4e2814b231705d8371dd7997f5b72e70baf/tcg/wasm32.c) — translation-block dispatch loop used as the vCPU pause boundary.
...
- [QEMU TCG CPU execution loop](https://github.com/ktock/qemu-wasm/blob/0ef7b4e2814b231705d8371dd7997f5b72e70baf/accel/tcg/cpu-exec.c) — normal translation-block exit and interrupt handling used to leave `cpu_exec()` before Linux Lab parks a browser vCPU.
- [QEMU MTTCG vCPU loop](https://github.com/ktock/qemu-wasm/blob/0ef7b4e2814b231705d8371dd7997f5b72e70baf/accel/tcg/tcg-accel-ops-mttcg.c) — outer vCPU-thread loop where Linux Lab waits after TCG execution has returned.
...
- The QEMU browser profile is explicitly single-vCPU. QEMU browser Pause freezes QEMU's CPU clock and elapsed-tick source, makes the qemu-wasm dispatcher return `TB_EXIT_REQUESTED`, leaves `cpu_exec()` through its normal interrupt path, and only then parks the outer vCPU thread with Emscripten's synchronous pthread sleep API. Resume restores the clocks before releasing the worker. This keeps the wait outside QEMU's setjmp/longjmp execution frame and avoids the blocking all-vCPU stop path.
