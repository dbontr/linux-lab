...
- [QEMU TCG CPU execution loop](https://github.com/ktock/qemu-wasm/blob/0ef7b4e2814b231705d8371dd7997f5b72e70baf/accel/tcg/cpu-exec.c) — `CPU_LOG_TB_NOCHAIN`, `CF_NO_GOTO_TB`, and `CF_NO_GOTO_PTR` provide bounded translation-block boundaries where the browser vCPU can observe a cooperative pause request without an asynchronous TCG exit.
...
- The QEMU browser profile is explicitly single-vCPU and runs with QEMU's `nochain` TCG mode. Linux Lab also disables indirect TCG chaining for that mode, giving the vCPU a bounded cooperative control point between translation blocks. Pause freezes QEMU's CPU clock and elapsed-tick source and parks the vCPU at that control point with WebAssembly atomic wait; Resume restores the clocks, clears the shared pause word, and notifies the worker. No Asyncify suspension, asynchronous `cpu_exit()`, signal, or blocking all-vCPU stop is required.
