...
- The QEMU browser profile is explicitly single-vCPU. Pause snapshots the adjusted virtual clock and elapsed ticks, sets a shared pause word, and parks the vCPU at qemu-wasm's C dispatcher with `emscripten_thread_sleep()`. Generated Wasm self-loops return to that dispatcher between iterations, while ordinary cross-TB chaining remains unchanged. The sleep loop services Emscripten proxy calls so Resume can advance the clock offsets and clear the pause word without Asyncify suspension. The pause path does not call `cpu_disable_ticks()`, `cpu_exit()`, `qemu_cpu_kick()`, or QEMU's stopped-vCPU wait.
