...
- [QEMU CPU exit request](https://github.com/ktock/qemu-wasm/blob/0ef7b4e2814b231705d8371dd7997f5b72e70baf/hw/core/cpu-common.c) — `cpu_exit()` interrupts linked TCG execution so a pause request reaches the qemu-wasm dispatcher boundary promptly without forcing a VM-wide blocking stop.
...
- The QEMU browser profile is explicitly single-vCPU. QEMU browser Pause freezes QEMU's CPU clock and elapsed-tick source, issues `cpu_exit()` to break linked TCG execution, then parks the vCPU between qemu-wasm translation blocks with Emscripten's synchronous pthread sleep API. Resume restores the clocks before releasing the worker. This avoids Asyncify suspension and QEMU's blocking all-vCPU stop path in the browser runtime.
