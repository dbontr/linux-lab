...
- [Emscripten 3.1.50 threading API](https://github.com/emscripten-core/emscripten/blob/3.1.50/system/include/emscripten/threading.h) — finite `emscripten_futex_wait()` and `emscripten_futex_wake()` calls used to park and release the QEMU vCPU worker without Asyncify suspension.
...
- The QEMU browser profile is explicitly single-vCPU. Both qemu-wasm TCI and generated direct/indirect TB chaining paths read Linux Lab's shared pause word before chaining. Direct `goto_tb` chaining falls through to its existing `exit_tb`; indirect `goto_ptr` uses the native null-TB exit. The QEMU CPU loop then parks the vCPU with finite Wasm futex waits until Resume restores the guest clocks, clears the pause word, and wakes the worker. Normal TB chaining stays enabled while running, and no Asyncify suspension, asynchronous active-vCPU exit, busy spin, or blocking all-vCPU stop is required.
