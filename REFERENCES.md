...
- [qemu-wasm Wasm/TCI dispatcher](https://github.com/ktock/qemu-wasm/blob/0ef7b4e2814b231705d8371dd7997f5b72e70baf/tcg/wasm32.c) — TCI direct and indirect chain edges are guarded by the shared pause word so Pause can end the current native chain instead of waiting for an unbounded hot loop.
- [qemu-wasm generated-TB backend](https://github.com/ktock/qemu-wasm/blob/0ef7b4e2814b231705d8371dd7997f5b72e70baf/tcg/wasm32/tcg-target.c.inc) — compiled direct and indirect chain emitters read the shared pause word and leave the generated TB on Pause; normal chaining remains unchanged while running.
- [QEMU TCG CPU execution loop](https://github.com/ktock/qemu-wasm/blob/0ef7b4e2814b231705d8371dd7997f5b72e70baf/accel/tcg/cpu-exec.c) — after a guarded chain exit, the single-vCPU execution loop reaches Linux Lab's worker-owned pause wait before selecting the next translation block.
...
- The QEMU browser profile is explicitly single-vCPU. Pause sets a shared word that is checked at generated-Wasm and TCI chain exits; a requested pause leaves the active chain and reaches the TCG CPU execution loop, where the vCPU snapshots adjusted clocks, publishes its stopped acknowledgement, and waits on that same word. Resume clears and notifies the word directly from JavaScript, then the worker updates per-clock offsets before executing the next translation block. The pause path does not call `cpu_disable_ticks()`, `cpu_exit()`, `qemu_cpu_kick()`, or QEMU's stopped-vCPU wait, and normal chaining remains unchanged while running.
