...
- QEMU browser Pause exits qemu-wasm at a translation-block boundary through QEMU's requested-exit contract, so QEMU synchronizes the exact guest PC and returns normally from `cpu_exec()`. The MTTCG thread then yields cooperatively with `emscripten_sleep()` outside the TCG/setjmp stack until Resume clears the pause flag.
