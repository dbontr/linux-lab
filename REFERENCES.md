...
- [qemu-wasm Wasm/TCI dispatcher](https://github.com/ktock/qemu-wasm/blob/0ef7b4e2814b231705d8371dd7997f5b72e70baf/tcg/wasm32.c) - `tcg_qemu_tb_exec()` is the stable C boundary between translated-block invocations. Linux Lab acknowledges Pause and waits on the shared pause word at that boundary without forcing QEMU out of `cpu_exec()`.
...
- [QEMU AIO bottom-half API](https://github.com/ktock/qemu-wasm/blob/0ef7b4e2814b231705d8371dd7997f5b72e70baf/include/block/aio.h) — the persistent text-input bottom half transfers browser keyboard commands onto QEMU's owning thread without cross-thread QEMU object allocation.
...
- [Emscripten 3.1.50 pthread sleep implementation](https://github.com/emscripten-core/emscripten/blob/3.1.50/system/lib/pthread/library_pthread.c) — `emscripten_thread_sleep()` services queued proxy calls on the main runtime pthread while using short futex-backed sleep slices at the QEMU dispatcher pause boundary.
- [ECMAScript Atomics](https://tc39.es/ecma262/multipage/fundamental-objects.html#sec-atomics-object) — `Atomics.load`, `Atomics.store`, and `Atomics.notify` provide the browser-thread shared-memory control operations for Pause, Resume, and state polling.
...
- The QEMU browser profile is explicitly single-vCPU. Browser Pause stores directly to a shared Wasm control word with JavaScript `Atomics`, so active QEMU execution does not service a cross-thread pause `ccall`. Generated Wasm self-loops return normally to qemu-wasm's C dispatcher; the vCPU snapshots adjusted guest clocks there, acknowledges the pause, and waits with `emscripten_thread_sleep()` so Emscripten proxy calls remain serviceable. Resume clears the shared word from JavaScript; the vCPU updates clock offsets before clearing its acknowledgment. Normal cross-TB chaining remains unchanged.
- Browser-thread Pause/Resume and run-state polling use preallocated shared control words only. The two word addresses are obtained once after QEMU reports ready; subsequent control actions use JavaScript `Atomics.load`, `Atomics.store`, and `Atomics.notify` without entering QEMU C code. Text input continues to use a persistent QEMU bottom half so guest input remains QEMU-owned.
