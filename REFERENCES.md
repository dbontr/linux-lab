...
- [QEMU vCPU lifecycle](https://github.com/ktock/qemu-wasm/blob/0ef7b4e2814b231705d8371dd7997f5b72e70baf/system/cpus.c) — native `cpu->stop`, `qemu_cpu_kick()`, and `cpu_resume()` behavior used to stop or wake the single browser vCPU without blocking the QEMU owner thread.
- [QEMU MTTCG vCPU loop](https://github.com/ktock/qemu-wasm/blob/0ef7b4e2814b231705d8371dd7997f5b72e70baf/accel/tcg/tcg-accel-ops-mttcg.c) — outer vCPU-thread loop where Linux Lab acknowledges and waits on a requested browser pause after QEMU has processed its normal stop state.
...
- [Emscripten 3.1.50 threading API](https://github.com/emscripten-core/emscripten/blob/3.1.50/system/include/emscripten/threading.h) — `emscripten_thread_sleep()` provides synchronous sleeping for pthread/Web Worker execution without using Asyncify suspension.
...
- The QEMU browser profile is explicitly single-vCPU. Pause freezes QEMU's CPU clock and elapsed-tick source, sets QEMU's native vCPU stop request, and kicks the vCPU so QEMU exits execution or wakes a halted CPU through its normal stop path. After QEMU records the vCPU as stopped, the outer MTTCG loop acknowledges the browser pause and waits with Emscripten's synchronous pthread sleep API. Resume clears QEMU's native stopped state before releasing that wait. This avoids custom translation-block/`cpu_exec()` interception and the blocking `pause_all_vcpus()` / `vm_stop()` path in the browser runtime.
